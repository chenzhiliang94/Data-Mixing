...
def generate_and_tokenize_prompt_headqa(data_point, tokenizer, add_eos_token, train_on_inputs):
    ...
    user_Q = {"role": "user", "content": data_point["qtext"] + "\n\n" + "Options: " + str(options)}
...
def generate_and_tokenize_prompt_mmlu(data_point, tokenizer, add_eos_token, train_on_inputs):
    
    # {'question': 'Glycogen breakdown in muscle initially results in the formation of:',
    ...
    def format(lst):
        ...
        for idx, option in enumerate(lst):
            answer = answer + ans[idx] + ". " + option + "\n"
        return answer
    
    options = data_point["choices"]
    right_answer = ans[data_point["answer"]] # A,B,C,D
    chat = []

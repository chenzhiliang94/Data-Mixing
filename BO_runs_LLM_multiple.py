...
parser.add_argument("--num_eval", help="number of eval domains",  type=int, default=2)
parser.add_argument("--experiments_setting", help="either ood or in_dist")
parser.add_argument("--output_dir", help="output_dir")
parser.add_argument("--data_selection", help="output_dir")
...
if setting == "ood": # take num eval number of task from each ood domain, iterate through all combi
    import itertools
    evaluation_weights = num_eval * [1/num_eval]
    data_domains_initial = list(task_metrics.keys())
    all_tasks_combi = itertools.combinations(data_domains_initial, num_eval) # size num_eval all combi
elif setting == "in_dist": # take num eval number of task from each ood domain, iterate through all combi
    import itertools
    evaluation_weights = num_eval * [1/num_eval]
    data_domains_initial = list(task_metrics.keys())
    all_tasks_combi = itertools.combinations(data_domains_initial, num_eval) # size num_eval all combi
            ...
            try:
                GP_input, observed_output, gp = run_BO_for_LLM(data_domains = data_domains,
                                                            random_dir = random_string, 
                                                                ...
                                                                sampling_method = data_selection, 
